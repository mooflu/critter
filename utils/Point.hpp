...
inline bool feq( const float &f1, const float &f2)
{
...
{
    Point2Di():x(0),y(0){}
    Point2Di( int d1, int d2):x(d1),y(d2){}
...
struct BoundingBox
{
    ...
    BoundingBox( const Point2D &p1, const Point2D &p2):_empty(false)
    {
...
struct vec4
{
    ...
    vec4( void):x(v[0]),y(v[1]),z(v[2]),a(v[3]) {}

    vec4( float X, float Y, float Z, float A):x(v[0]),y(v[1]),z(v[2]),a(v[3]) { x=X; y=Y; z=Z; a=A;} 
    vec4(const vec4 &vi):x(v[0]),y(v[1]),z(v[2]),a(v[3]) 
    {
	v[0]=vi.x; 
	v[1]=vi.y; 
	v[2]=vi.z; 
