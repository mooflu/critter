...
public:
    DimensionObject(int w, int h) :
        _dim(w, h) {}

    virtual ~DimensionObject() {}

    virtual const Dimension& dimension(void) const { return _dim; }
...
public:
    ...
    const Point2Di& LastOffset(void) { return _lastOffset; }

    int PercentUsed(void) const { return (_areaUsed * 100) / _totalArea; }
